...
class DataGenerator(kutils.Sequence):
    ...
    def __init__(self, in_file_path, batch_size=32, ddepth=48000, shuffle=True, other_class_cnt=25000):
    ...
    def list_inp_files_and_output(self):
        ...
        cnt_oth = 0
        for fl in inpfiles:
            if re.match(r'(?i)^.+_question.wav$', fl):
                self.in_file_names.append(fl)
                self.output_arr[fl] = [1, 0, 0]
            elif re.match(r'(?i)^.+_exclamation.wav$', fl):
                self.in_file_names.append(fl)
                self.output_arr[fl] = [0, 1, 0]
            elif re.match(r'(?i)^.+_other.wav$', fl):
