...
def cut_movie(line,count,movie_path,sub):
    ...
    with VideoFileClip(movie_path) as video:
        ...
        if ('?' in sub):
            ...
            new.write_videofile("/work/video_datasets/question/" + movie_set_folder + "_" + "movie"+ str(movie_no) + "_vid" + str(count) + ".mp4")
            ...
            input_path = "/work/video_datasets/question/" + movie_set_folder + "_" +"movie"+str(movie_no)+"_vid"+ str(count) + ".mp4"
            output_path = "/work/video_datasets/question/" + movie_set_folder + "_"  + "movie" + str(movie_no) + "_aud" + str(count) + ".mp3"
            command = "ffmpeg -i " + input_path + " -b:a 16000 -vn " + output_path
            subprocess.call(command,shell=True)
        elif ('!' in sub):
            new.write_videofile("/work/video_datasets/exclamation/" + movie_set_folder + "_" + "movie"+ str(movie_no) + "_vid" + str(count) + ".mp4")
            ...
            input_path = "/work/video_datasets/exclamation/" + movie_set_folder + "_"  + "movie" + str(movie_no)+"_vid"+ str(count) + ".mp4"
            output_path = "/work/video_datasets/exclamation/" + movie_set_folder + "_" + "movie" + str(movie_no) + "_aud" + str(count) + ".mp3"
            command = "ffmpeg -i " + input_path + " -b:a 16000 -vn " + output_path
            subprocess.call(command,shell=True)
        else:
            new.write_videofile("/work/video_datasets/other/" + movie_set_folder + "_" + "movie"+ str(movie_no) + "_vid" + str(count) + ".mp4")
            ...
            input_path = "/work/video_datasets/other/" + movie_set_folder + "_" +"movie"+str(movie_no)+"_vid"+ str(count) + ".mp4"
            output_path = "/work/video_datasets/other/" + movie_set_folder + "_" + "movie" + str(movie_no) + "_aud" + str(count) + ".mp3"
            command = "ffmpeg -i " + input_path +" -b:a 16000 -vn " + output_path
            subprocess.call(command,shell=True)
        ...
        while(line):
            if '-->' in line:
                ...
                while(line!=""):
                    ...
                    line = line.strip()
                if ('?' in sub):
                    txt_file = open("/work/video_datasets/question/"+ movie_set_folder + "_"+"movie"+str(movie_no)+"_sub"+str(count)+".txt","w")
                elif ('!' in sub):
                    txt_file = open("/work/video_datasets/exclamation/"+ movie_set_folder + "_" +"movie"+str(movie_no)+"_sub"+str(count)+".txt","w")
                else :
                    txt_file = open("/work/video_datasets/other/"+ movie_set_folder + "_" +"movie"+str(movie_no)+"_sub"+str(count)+".txt","w")
                txt_file.write(sub)
...
if __name__ == '__main__':
    ...
    for dirs in movies_list:
        if os.path.isdir(movie_dir+dirs):
            ...
            for file in (glob.glob("*.mp4") or glob.glob("*.mkv") or glob.glob("*.avi")):
